import { browseDialogLabel, browseDialogPages, createMenuScreenComponent, reviewDialogPages, safeMenuText, } from "./components/index.js";
...
async function runTuiMenu(ctx, definition, options) {
    ...
    try {
        while (!navigator.closed) {
            ...
            const event = await showTuiScreen(ctx, screen, navigator.selectionFor(navigator.current, selectableItemIds(screen)), menuSignal, {
...
async function showTuiScreen(ctx, screen, selectedItemId, menuSignal, callbacks) {
    let component;
