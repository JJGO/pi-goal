...
function formatReviewLines(screen, width, theme) {
    const segments = reviewSegments(screen.content, width);
    const format = screen.format ?? { kind: "text" };
    if (format.kind === "code") {
        const language = format.language ?? (format.filePath ? getLanguageFromPath(format.filePath) : undefined);
        return segments.map(({ text }) => theme.fg("mdCodeBlock", highlightCode(text, language)[0] ?? text));
    }
    if (format.kind === "diff") {
...
function plainReviewLines(content, width) {
    return reviewSegments(content, width).map(({ text }) => text);
}
function reviewSegments(content, width) {
